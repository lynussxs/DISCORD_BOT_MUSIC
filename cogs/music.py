...
class WebshareProxyManager:
    ...
    def rotate(self) -> str:
        """Chuyển proxy tiếp theo khi bị chặn."""
        if not self._proxies:
            self._fetch()
        if self._proxies:
...
class Track:
    ...
    async def from_query(
        ...
        # Nếu vẫn fail thì rotate proxy và thử lại
        for attempt in range(3):
            ...
            try:
                ...
                is_retryable = any(x in err_str for x in [
                    "Requested format", "Sign in", "cookies", "bot", "403", "Connection"
                ])
                if is_retryable and attempt < 2:
                    # Rotate sang proxy mới
                    new_proxy = _proxy.rotate()
                    logger.warning("Proxy failed, rotating to: %s (attempt %d)", new_proxy[:30], attempt + 1)
                    continue
                raise
