...
YTDL_OPTIONS: dict[str, Any] = {
    ...
    "proxy"           : "http://fywznozi:gv94cmc9t7qs@142.111.67.146:5611",  # ⚠️ THAY PROXY NẾU CẦN
    "extractor_args": {
        "youtube": {
...
# Options không dùng cookies — fallback khi cookies hết hạn
YTDL_OPTIONS_NO_COOKIE: dict[str, Any] = {
    "format"          : "249/139/251/140/18/bestaudio/best",
    "default_search"  : "ytsearch",
    "noplaylist"      : False,
    "quiet"           : True,
...
class Track:
    ...
    async def from_query(
        ...
        # Nếu fail (cookies hết hạn / format không có) thì fallback không cookies
        try:
            ...
            err_str = str(e)
            if "Requested format" in err_str or "Sign in" in err_str or "cookies" in err_str.lower():
                # Fallback: thử không cookies
                with yt_dlp.YoutubeDL(YTDL_OPTIONS_NO_COOKIE) as ytdl:
                    partial = functools.partial(ytdl.extract_info, video_url, download=False)
