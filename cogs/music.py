...
# ── yt-dlp ─────────────────────────────────────────────────────────────────────
#
# format: prefer webm audio (usually the highest quality stream on YouTube
#         without needing video).  Falls back to any best audio, then best.
#
# default_search: "ytsearch" handles bare text queries; explicit URLs bypass
#                 this.  from_query() also manually prepends "ytsearch:" for
...
class Track:
    ...
    async def make_source(self, volume: float) -> discord.FFmpegOpusAudio:
        ...
        return discord.FFmpegOpusAudio(
            ...
            options=f"-vn -filter:a volume={safe_vol:.3f}",
        )
...
def _e_np(
    ...
    """
    Full 'Now Playing' embed used by both the player loop and /nowplaying.
    Uses set_image() so the thumbnail is displayed large (full width).
    """
    ...
    e.add_field(name="In Queue",     value=f"`{q_len}` track(s)",     inline=True)
    e.add_field(
        name  = "Progress",
    ...
    e.add_field(
        name  = "Volume",
        value = _vol_bar(vol_pct),
        inline= False,
    )
    if track.thumbnail:
        e.set_image(url=track.thumbnail)      # large full-width thumbnail
    e.set_footer(text=track.uploader)
    return e
...
class MusicControlView(discord.ui.View):
    """
    Five-button control panel attached to every 'Now Playing' message.

    Layout
    ───────
      Row 0:  ⏯ Pause/Resume  |  ⏭ Skip  |  ⏹ Stop
      Row 1:  🔉 −10%         |  🔊 +10%

    State rules
    ────────────
    • Pause/Resume, Skip, and Stop are disabled when no track is current.
    • Volume buttons are always enabled (they set the level for the next track).
    • _sync() must be called before any edit_message() to keep states current.

    Volume note
    ────────────
    Because volume is baked into the FFmpeg filter at source-creation time
    (system libopus is absent, so PCMVolumeTransformer is not available),
    volume changes take effect at the start of the NEXT track.  The embed
    reflects the new target immediately so the UI stays responsive.
    """

    def __init__(self, player: "GuildPlayer") -> None:
        # 24-hour timeout: buttons persist for the length of any reasonable
        # music session without requiring the persistent-view infrastructure.
    ...
    def _sync(self) -> None:
        """Update disabled/emoji/label on every button from current player state."""
        has    = self.player.current is not None
        paused = (
            self.player.vc.is_paused()
        ...
        for item in self.children:
            ...
            elif cid in ("music_skip", "music_stop"):
                item.disabled = not has
            # volume buttons: always enabled — changes pending vol for next track

    def _build_embed(self) -> discord.Embed:
        """Return the appropriate embed for the current player state."""
        p = self.player
        if p.current:
            return _e_np(
                p.current,
    ...
    async def _quick_edit(self, interaction: discord.Interaction) -> None:
        """Edit-in-place after a fast synchronous action (pause/resume/volume)."""
        self._sync()
    ...
    async def btn_pp(
        ...
        if p.vc.is_paused():
            p.resume()
            logger.info("BUTTON pause/resume → resumed [guild %d]", p.vc.guild.id)
        else:
            p.pause()
            logger.info("BUTTON pause/resume → paused [guild %d]", p.vc.guild.id)
    ...
    async def btn_skip(
        ...
        # Disable controls on this (now-finished) NP card.
        # The player loop will post a fresh NP card for the next track.
        self._disable_all()
    ...
    @discord.ui.button(
        ...
        custom_id="music_stop", row=0,
    )
    async def btn_stop(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ) -> None:
        # Edit the message immediately (disable all buttons, show stopping state).
        # This must happen before the async stop() so the interaction doesn't
        # expire while we wait for the voice disconnect.
        self._disable_all()
        await interaction.response.edit_message(
            embed=discord.Embed(
                title       = "⏹  Stopping…",
        ...
        # on_voice_state_update will clean up the _players dict.

    @discord.ui.button(
        emoji="🔉", label="−10%",
        style=discord.ButtonStyle.secondary,
        custom_id="music_vdn", row=1,
    )
    async def btn_vol_down(
        ...
        logger.info(
            "BUTTON vol − → %d%% [guild %d]",
            new_pct, self.player.vc.guild.id,
        )
        await self._quick_edit(interaction)
